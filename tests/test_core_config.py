...
"""
    ...
    def test_substitute_env_vars_nested(self):
        ...
        with patch.dict(os.environ, {"VAR1": "val1", "VAR2": "val2"}):
            data = {
                "level1": {
                    "level2": ["${VAR1}", {"level3": "${VAR2}"}]
                }
            }
            result = ClientConfig._substitute_env_vars(data)
            assert result == {
                "level1": {
                    "level2": ["val1", {"level3": "val2"}]
                }
            }
    ...
    def test_save_default_path(self, tmp_path):
        ...
        with patch.object(ClientConfig, 'get_default_config_file', return_value=tmp_path / "default.json"):
            config = ClientConfig()
            config.save()
    ...
    def test_load_default_path(self, tmp_path):
        ...
        with patch.object(ClientConfig, 'get_default_config_file', return_value=default_path):
    ...
    def test_get_default_config_file(self):
        ...
        with patch('retunnel.core.config.get_config_dir', return_value=Path("/test/config")):
            config = ClientConfig()
...
class TestAuthConfig:
    ...
    def test_init_with_existing_file(self, tmp_path):
        ...
        config_data = {"auth_token": "existing-token", "api_key": "existing-key"}
